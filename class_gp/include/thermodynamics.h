...
enum reionization_parametrization {
                                   ...
                                   reio_gpr_tanh    /**< new GPR-based tanh reionization scheme Hanyu */ 
};
...
/* Hanyu */
/***************************************************************
 *                 STRUCTURES NEEDED FOR GPR (reio_gpr_tanh)
 ***************************************************************/

/**
 * Structure containing kernel hyperparameters for the GP model.
 * For a simple RBF kernel: k(z1,z2)=sigma_f^2 * exp(-(z1-z2)^2/(2*l^2))
 */
typedef struct {
  double sigma_f; /**< kernel amplitude */
  double l;        /**< kernel length scale */
} kernel_hyperparameters;

/**
 * Gaussian Process model structure to store the trained GP.
 *
 * - nt: number of training points
 * - z_train: array of training redshifts (monotonically increasing)
 * - x_train: array of training x_e values at these redshifts
 * - invK: inverse of the kernel covariance matrix (nt-by-nt)
 * - mean_function: pointer to mean function, or zero if mean_is_zero=1
 * - kernel: pointer to kernel function (e.g., RBF)
 * - kernel_hyperparameters: pointer to kernel hyperparameters structure
 * - mean_is_zero: flag if mean function is zero
 */
typedef struct {
  int nt;                            /**< number of training points */
  double *z_train;                   /**< training redshifts */
  double *x_train;                   /**< training ionization fractions */
  double *invK;                      /**< inverse kernel matrix */
  double (*mean_function)(double z); /**< mean function pointer */
  double (*kernel)(double z1, double z2); /**< kernel function pointer */
  void *kernel_hyperparameters;      /**< pointer to kernel hyperparameters */
  int mean_is_zero;                  /**< boolean: 1 if mean=0, else 0 */
} gp_model;
/* Hanyu */

/***************************************************************
 *                 MAIN THERMODYNAMICS STRUCTURES
 ***************************************************************/
...
/* Hanyu */
  /** parameters for reio_gpr_tanh */
  /** int gpr_n_interp;       /**< number of interpolation points for GPR */
  int gpr_n_low;          /**< number of interpolation points in low redshift region for GPR*/

  int gpr_n_high;         /**< number of interpolation points in high redshift region for GPR*/ 

  int gpr_reio_num;           /**< number of GPR reionization control points */

  double *gpr_reio_z;         /**< array of redshifts for GPR-based reionization */

  double *gpr_reio_xe;        /**< initial values for x_e at these redshifts */

  double gpr_reio_step_sharpness; /**< sharpness of the tanh transitions in GPR scheme */

  double gpr_sigma_f; /**< GP kernel sigma_f hyperparameter read from input */

  double gpr_l;       /**< GP kernel l hyperparameter read from input */

  double gpr_z_min;       /**< minimum redshift for GPR interpolation */

  double gpr_z_max;       /**< maximum redshift for GPR interpolation */

  double gpr_z_transition; /**< redshift value where interpolation interval size changes for GPR*/

  /* Optical depth calculation parameters */
  double z_c;       /* Dividing redshift for optical depth calculation */

  double z_max;     /* Maximum redshift for high-z optical depth calculation */

  double tau_lowz;  /* Optical depth for 0 < z < z_c */

  double tau_highz; /* Optical depth for z_c < z < z_max */

  double tau_total; /* Total optical depth for 0 < z < z_max */

  /* Pointer to GP model structure */
  gp_model *gp_reio_model;         /**< pointer to trained GP model */
/* Hanyu */

  struct thermo_reionization_parameters *ptrp;    /* Pointer to reionization parameters */

  short is_allocated; /* Added dummy field to avoid compilation error with classy.cpp */
...
/* Hanyu */
  /* New indices for reio_gpr_tanh */
  int index_re_step_sharpness_gpr;  /**< index for gpr step sharpness parameter */
  int index_re_xe_before_gpr;       /**< index for xe_before in gpr scheme */
/* Hanyu */
